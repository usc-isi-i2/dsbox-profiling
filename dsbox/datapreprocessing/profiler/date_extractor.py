"""
Extracted from https://github.com/usc-isi-i2/etk/tree/development
"""
...
from . import date_regex_generator
...
class DateExtractor():
    def __init__(self, etk=None, extractor_name: str='date extractor') -> None:
        ...
        d = date_regex_generator.DateRegexGenerator(constants.singleton_regex, constants.units)
    ...
    def extract(self, text: str = None,
                ...
                prefer_language_date_order: bool = False,
                timezone: str = None,
        ...
        if additional_formats:
            for date_format in additional_formats:
                order = ''
                reg = date_format
                for key in constants.singleton_regex:
                    if key[0] == '%':
                        reg2 = re.sub(key, constants.singleton_regex[key], reg)
                        if reg != reg2:
                            if key in constants.units['M']:
                                order += 'M'
                            elif key in constants.units['Y']:
                                order += 'Y'
                            elif key in constants.units['D']:
                                order += 'D'
                            reg = reg2
    ...
    def remove_overlapped_date_str(self, results: List[List[dict]]) -> List[datetime.datetime]:
        """
            ...
            if cur_max['end'] <= x['start']:
                ...
                if len(x['value']) > len(cur_max['value']):
                    ...
                    if x['order'] in ['SINGLE_YEAR']:
                        ...
                        if self.settings[PREFER_LANGUAGE_DATE_ORDER] and self.lan in constants.language_date_order:
                            if x['order'] == constants.language_date_order[self.lan]:
                                cur_max = x
    ...
    def parse_date(self, date_info: dict) -> datetime.datetime or None:
        ...
        if date_info['pattern']:
            ...
            if re.match(constants.possible_illegal, date_info['value']):
                return None
        ...
        for s in date_info['groups']:
            if s:
                ...
                if p in constants.units['D']:
                    miss_day = False
                if p in constants.units['M']:
                    miss_month = False
                if p in constants.units['Y']:
                    miss_year = False
                if p in constants.units['W']:
                    miss_week = False
                pattern.append(p)
                formatted_str = s.strip('.').strip().lower()
                if p in ['%B', '%b', '%A', '%a']:
                    if formatted_str in constants.foreign_to_english:
                        formatted_str = constants.foreign_to_english[formatted_str]
                if p in ['%b', '%a']:
        ...
        if formatted and pattern:
            ...
            if miss_year and miss_month and miss_day:
                ...
                if miss_week:
                    ...
                    if week_of_day in constants.foreign_to_english:
                        week_of_day = constants.foreign_to_english[week_of_day]
                    target = constants.day_of_week_to_number[week_of_day] if week_of_day in constants.day_of_week_to_number \
                        else today.weekday()
    ...
    def extract_relative_dates(self, text: str) -> None:
        """
